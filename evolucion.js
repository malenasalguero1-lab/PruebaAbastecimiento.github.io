(function() {
  ...
  =========================== */
  window.initEvolucion = function() {
    if (window.evolucionInitialized) return;
    window.evolucionInitialized = true;

    // Use fetchWithCache for faster loading from IndexedDB
    fetchWithCache(EVOL_URL + "?t=" + window.CACHE_BUSTER)
      .then(text => {
        ...
        const obraSel = document.getElementById("mm_obraSelect");
        ...
        function render() {
          const obrasSel = getSelValues("mm_obraSelect");
          ...
          const seriesBars = stackCols.map(c => {
            const n = norm(c);
            const COLORS = {
              [norm("Cantidad Menor a PP")]: "#1d4ed8",
              [norm("Cantidad Mayor a PP")]: "#16a34a",
              [norm("Cantidad mayor a Stock Maximo")]: "#f59e0b",
              [norm("Cantidad Stock Nulo")]: "#d32f2f"
            };
          ...
          const el = document.getElementById("mm_chartEvolucion");
          if (!el) return;
        ...
        if (obraSel) {
          obraSel.addEventListener("change", (e) => {
